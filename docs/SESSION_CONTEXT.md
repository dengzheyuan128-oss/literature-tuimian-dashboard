...
**正在进行**: SOP v2.0和Schema v1.2已发布，支持夏令营+预推免双通知类型

**当前分支**: `main`

**最后提交**: `2c6e07e` - docs: update SOP to v2.0 and schema to v1.2 for dual notice types
...
1. **按SOP v2.0处理20所待补充院校**
   - 使用合并关键词搜索：`site:域名 2026 (推免 OR 夏令营)`
   - 每所≤5分钟，同时查找夏令营和预推免

### 接下来执行（P1）
2. **更新现有130所院校的通知类型**
   - 区分夏令营vs预推免
   - 添加noticeType和sourceChannel字段
