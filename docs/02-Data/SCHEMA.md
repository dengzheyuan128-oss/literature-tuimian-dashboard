...
**双通知类型支持**
- 新增 `noticeType` 字段：支持夏令营/预推免/综合/未找到
- 新增 `sourceChannel` 字段：标注链接来源渠道
- 支持每个program收集两种通知类型
- 配合SOP v2.0使用
...
| `noticeType` | string | 通知类型（见下方说明） |
| `year` | number | 推免年份 |
| `title` | string | 通知标题 |
| `url` | string | 通知链接 |
| `sourceType` | string | 来源类型 |
| `sourceChannel` | string | 来源渠道（见下方说明） |
...
### noticeType（通知类型）- v1.2新增

**合法值**：
- `"夏令营"` - 暑期夏令营/优才夏令营（通常5-7月发布，针对下一年推免）
- `"预推免"` - 预推免/推免接收（通常8-10月发布）
- `"综合"` - 夏令营与预推免合并通知
- `"未找到"` - 该类型通知未找到或尚未发布

**重要说明**：
- 夏令营和预推免是两种**独立且同等重要**的通知类型
- 每个program应尝试收集两种类型的通知
- 部分院校只发布其中一种，另一种标记为"未找到"
- 夏令营的时效性通常早于预推免

### sourceChannel（来源渠道）- v1.2新增

**合法值**：
- `"官网原文"` - 官方域名直接访问获得
- `"公众号原文"` - mp.weixin.qq.com（时效性强可接受）
- `"第三方平台"` - 考研帮、保研论坛等（需额外验证）
- `"搜索引擎"` - 通过site:搜索或扩展搜索发现
- `"待查"` - 三渠道均无结果，建议电话咨询
