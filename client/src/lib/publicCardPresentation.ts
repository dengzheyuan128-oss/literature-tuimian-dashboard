...
export function getNextActionLabel(
  card: Pick<PublicProgramCard, "deadline" | "sourceUrl" | "url">,
  now: Date = new Date(),
): string {
  const sourceUrl = card.sourceUrl || card.url || "";
  if (!sourceUrl) return "等待补充官方来源";
...
function parseSupportedDate(value: string): Date | null {
  ...
  if (slash) {
    return new Date(Number(slash[1]), Number(slash[2]) - 1, Number(slash[3]));
  }
